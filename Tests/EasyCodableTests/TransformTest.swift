...
class TransformTest: XCTestCase {
    ...
    struct ExampleModel: Codable {
        @EasyCodable(transformer: TransformOf<ValueWrapper, String>(fromJSON: { ValueWrapper(value: $0) }, toJSON: { $0.value }))
        var valueA = ValueWrapper(value: "A")

        @EasyCodable(transformer: TransformOf<ValueWrapper?, String>(fromJSON: { ValueWrapper(value: $0) }, toJSON: { $0?.value }))
        var valueB = ValueWrapper(value: "B")

        @EasyCodable(transformer: TransformOf<ValueWrapper?, String>(fromJSON: { $0 != nil ? ValueWrapper(value: $0) : nil }, toJSON: { $0?.value }))
    ...
    func testCustomUnOptionalTransform() throws {
        struct ExampleModel: Codable {
            @EasyCodable(transformer: EnumInt.transformer)
    ...
    func testCustomOptionalTransform() throws {
        struct ExampleModel: Codable {
            @EasyCodable(transformer: EnumInt.optionalTransformer)
            var one: EnumInt = .three
    ...
    func testCustomTransformCodec() throws {
        struct ExampleModel: Codable {
            ...
            @EasyCodable("tuple" ?? "tp", transformer: tupleTransform)
            var tuple: (String, String)?

            @EasyCodable(transformer: tupleTransform)
...
//        XCTAssertEqual(String(data: jsonData, encoding: .utf8), "{\"id\":1,\"tuple\":\"left|right\"}")
    ...
    func testDateTransfrom() throws {
        struct ExampleModel: Codable {
            @EasyCodable(transformer: SecondDateTransform())
            var sencondsDate: Date?

            @EasyCodable(transformer: MillisecondDateTransform())
